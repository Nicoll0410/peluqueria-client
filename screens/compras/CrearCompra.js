    ...
    const fetchData = async () => {
      try {
        ...
        const [provRes, insRes] = await Promise.all([
          axios.get('https://peluqueria-server-gw54.onrender.com/proveedores/all', {
            headers: { Authorization: `Bearer ${token}` }
          }),
          axios.get('https://peluqueria-server-gw54.onrender.com/insumos/all', {
            headers: { Authorization: `Bearer ${token}` }
          })
        ]);
...
const compraData = {
  ...
  insumos: formData.insumos.map(i => ({
    ...
    precio_unitario: Number(i.precioUnitario)
      ...
      const response = await axios.post('https://peluqueria-server-gw54.onrender.com/compras', compraData, {
        headers: { Authorization: `Bearer ${token}` }
      });
